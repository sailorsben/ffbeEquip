...
const unitRules = {
    ...
    9516: (item) => item.exclusiveUnits = ["401015507","401015517"] // Halloween Chow
}
...
function manageRequirement(skill, debugItems, copy) {
    let requirements = skill.requirements;
    if (!Array.isArray(requirements[0])) requirements = [requirements];
    if (debugItems.includes(copy.id)) console.log("requirements for item", copy.name, JSON.stringify(requirements));
    requirements.forEach(req => {
        if (req[0] === 'UNIT_ID') {
            var unitsFound = [];
            for (var restrictedUnitIndex in req[1]) {
                if (unitNamesById[req[1][restrictedUnitIndex]]) {
                    unitsFound.push(req[1][restrictedUnitIndex].toString());
                }
            }
...
function addEffectToItem(item, skill, rawEffectIndex, skills) {
    ...
    // Auto- abilities
        ...
        if (skillIn) {
            var autoCastedSkill = parseActiveSkill(rawEffect[3][0].toString(), skillIn, skills, item);
    ...
    // Auto- abilities
        ...
        if (skillIn) {
            var counterSkill = parseActiveSkill(rawEffect[3][2].toString(), skillIn, skills, item);
    ...
    // Break, stop and charm resistance with turn number
        ...
        let skill = parseActiveSkill(rawEffect[3][2], skills[rawEffect[3][2]], skills, item);
        var magicType = "";
        if (rawEffect[3][0] ==  0) {
...
function parseActiveSkill(skillId, skillIn, skills, item, enhancementLevel = 0) {
    ...
    for (var rawEffectIndex in skillIn["effects_raw"]) {
        ...
        var effect = parseActiveRawEffect(rawEffect, skillIn, skills, item, skillId, enhancementLevel);
        if (effect && effect.id) {
...
function parseActiveRawEffect(rawEffect, skillIn, skills, item, skillId, enhancementLevel = 0) {
    var result = null;
    ...
    // Randomly use skills
        ...
        for (var i = 0, len = rawEffect[3].length; i < len; i++) {
            ...
            if (data && data[0] && skills[data[0]]) {
                ...
                var skill = parseActiveSkill(data[0], skillIn, skills, item);
    ...
    // Cooldown skills
        ...
        if (skillIn) {
            result = {};
            result.cooldownSkill = parseActiveSkill(id.toString(), skillIn, skills, item);
            result.cooldownTurns = rawEffect[3][2][0] + 1;
            result.startTurn = result.cooldownTurns - rawEffect[3][2][1];
            if (rawEffect[3][3] == 0) {
                if (result.cooldownSkill.effects.some(e => e.effect && e.effect.damage && e.effect.damage.mechanism == 'physical')) {
    ...
    // Conditional skills
        ...
        var skill1 = parseActiveSkill(skillId1, skillIn1, skills, item, enhancementLevel);

        var skillId2 = rawEffect[3][3].toString();
        var skillIn2 = skills[skillId2];
        var skill2 = parseActiveSkill(skillId2, skillIn2, skills, item, enhancementLevel);
        ...
        addUnlockedSkill(skillId1, skill2, item);
        return skill1;
    ...
    // Magical Damage
        ...
        result = {"damage":{"mechanism":"magical", "damageType":"mind", "coef":rawEffect[3][5]/100}};
    ...
    // Physical Damage
        ...
        result = {"damage":{"mechanism":"physical", "damageType":"body", "coef":rawEffect[3][6]/100}};
    ...
    // Physical Damage with ignore DEF
        ...
        result = {"damage":{"mechanism":"physical", "damageType":"body", "coef":rawEffect[3][2]/100, "ignore":{"def":-rawEffect[3][3]}}};
    ...
    // Magical Damage with ignore SPR
        ...
        result = {"damage":{"mechanism":"magical", "damageType":"mind", "coef":rawEffect[3][2]/100, "ignore":{"spr":rawEffect[3][3]}}};

    // Physical Damage from DEF
    } else if (rawEffect[2] == 102) {
        result = {"damage":{"mechanism":"physical", "damageType":"body", "coef":rawEffect[3][2]/100, use: {"stat":"def", "percent":rawEffect[3][0], "max":rawEffect[3][1]}}};

    // Magical Damage from SPR
    } else if (rawEffect[2] == 103) {
        result = {"damage":{"mechanism":"magical", "damageType":"mind", "coef":rawEffect[3][2]/100, use: {"stat":"spr", "percent":rawEffect[3][0], "max":rawEffect[3][1]}}};

    // Magical Damage with stacking
    } else if (rawEffect[2] == 72) {
        result = {"damage":{"mechanism":"magical", "damageType":"mind", "coef":(rawEffect[3][2] + rawEffect[3][3])/100, "stack":rawEffect[3][4]/100, "maxStack":rawEffect[3][5] - 1}};

    // Physical Damage with stacking
    } else if (rawEffect[2] == 126) {
        result = {"damage":{"mechanism":"physical", "damageType":"body", "coef":(rawEffect[3][3] + rawEffect[3][4])/100, "stack":rawEffect[3][5]/100, "maxStack":rawEffect[3][6] - 1}};
    ...
    // Jump damage
        ...
        result = {"damage":{"mechanism":"physical", "damageType":"body", "coef":rawEffect[3][4]/100, "jump":true, delay:rawEffect[3][3]}};
    ...
    // Delayed damage
        ...
        result = {"damage":{"mechanism":"physical", "damageType":"body", "coef":rawEffect[3][5]/100, delay:rawEffect[3][0]}};
    ...
    // Timed Jump
        ...
        result = {"damage":{"mechanism":"physical", "damageType":"body", "coef":rawEffect[3][4]/100, "jump":true, delay:rawEffect[3][2]}};
    ...
    // Combo damage
        ...
        result = {"damage":{"mechanism":"physical", "damageType":"body", "coef":rawEffect[3][4]/100, "combo": true, "minTime":rawEffect[3][2], "maxTime":rawEffect[3][3]}};
    ...
    // Damage increased against a race
        ...
        result = {"damage":{"mechanism":"physical", "damageType":"body", "coef":1, "ifUsedAgain":{"race":raceMap[rawEffect[3][0]], "coef":rawEffect[3][3]/100}}};

    // Critical Physical Damage
    } else if (rawEffect[2] == 43) {
        result = {"damage":{"mechanism":"physical", "damageType":"body", "coef":rawEffect[3][2]*1.5/100}};
    ...
    // Gain Skill
        ...
        addUnlockedSkill(gainedSkillId, multicastskill, item, skillId);
    ...
    // Gain Skill
        ...
        for (var i = 0, len = gainedSkillIds.length; i < len; i++) {
            ...
            result.gainSkills.skills.push(parseActiveSkill(gainedSkillIds[i].toString(), gainedSkill, skills, item));
        }
    ...
    // Gain Multicast ability
        ...
        if (gainedSkill) {
            ...
            addUnlockedSkill(parsedSkill.id, parsedSkill, item, skillId);
    ...
    if (result && result.damage) {
        ...
        if (result.damage.mechanism == "magic") {
...
function addUnlockedSkill(gainedSkillId, gainedSkill, item, unlockedBy) {
    if (!item.skills) item.skills = [];

    var alreadyAdded = false;
    for (var i = item.skills.length; i--;) {
        if (item.skills[i].id == gainedSkillId) {
            alreadyAdded = true;
            if (unlockedBy) {
                if (!item.skills[i].unlockedBy) {
                    item.skills[i].unlockedBy = [];
                }
                if (!item.skills[i].unlockedBy.includes(unlockedBy.toString())) {
                    item.skills[i].unlockedBy.push(unlockedBy.toString());
                }
